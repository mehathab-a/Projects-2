...
def segment_leaf(image_file, filling_mode, smooth_boundary, marker_intensity):
    ...
    largest_mask = \
        select_largest_obj(bin_image, original, fill_mode=filling_mode,
    ...
    # image_original = cv2.cvtColor(original, cv2.COLOR_RGB2GRAY)
    bitwise_mask = cv2.bitwise_and(image, original)
    output_image = bitwise_mask
